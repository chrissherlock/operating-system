#!/usr/bin/env python3
# =====================================================================
# fix.py: Remove enclosing card styling from System Architecture Overview
# =====================================================================
import os
import subprocess
...
def remove_architecture_card_styling():
    ...
    # Old card-wrapped architecture block
    old_arch_card = """      <!-- System Architecture Overview -->
    ...
    # New plain text flow markup (no background card, standard h2 and paragraph styling)
    new_arch_inline = """      <!-- System Architecture Overview -->
    ...
    if old_arch_card in content:
        content = content.replace(old_arch_card, new_arch_inline)
        print("--> Successfully removed card wrapper from architecture overview.")
    else:
        print("--> Warning: Exact old card block not matched. Performing substring search...")
        # Fallback: find <h2>System Architecture Overview</h2> and replace surrounding card markup
        if "System Architecture Overview" in content:
            # Let's target the card div boundaries if exact match failed
            parts = content.split("<!-- System Architecture Overview -->")
            if len(parts) >= 2:
                # Reconstruct cleanly
                sub_parts = parts[1].split("</div>\n      </div>")
                if len(sub_parts) >= 2:
                    content = parts[0] + new_arch_inline + "".join(sub_parts[1:])
                    print("--> Replaced via comment boundary split.")
    ...
    try:
        ...
        commit_msg = (
            "Remove card styling from System Architecture Overview in Module 2\n\n"
            "Strip out the enclosing card container and background from the System Architecture\n"
            "Overview diagram in 02-hardware-review.html so it flows directly as inline body text."
        )
        subprocess.run(["git", "commit", "-m", commit_msg], check=True)
        subprocess.run(["git", "push", "origin", "main"], check=True)
        print("--> Git sync completed successfully for card removal!")
    except Exception as e:
        print(f"Git execution note: {e}")

if __name__ == "__main__":
    remove_architecture_card_styling()
