#!/usr/bin/env python3
# =====================================================================
# fix.py: Generate complete 02-hardware-review.html with unified container
# =====================================================================
import os
import subprocess
...
def generate_hardware_review():
    with open(TARGET_FILE, "w", encoding="utf-8") as f:
        f.write(FULL_HTML)
    print(f"--> Overwrote {TARGET_FILE} with clean single-card container structure.")

    try:
        subprocess.run(["git", "add", "fix.py", TARGET_FILE], check=True)
        commit_msg = (
            "Unify Module 2 content within single container card without nested header card\n\n"
            "Remove inner card border, background, and padding from header in\n"
            "02-hardware-review.html so header and body text flow seamlessly\n"
            "inside the main .container card, matching Module 3."
        )
        subprocess.run(["git", "commit", "-m", commit_msg], check=True)
        subprocess.run(["git", "push", "origin", "main"], check=True)
        print("--> Git sync completed successfully for Module 2 single-card structure!")
    except Exception as e:
        print(f"Git execution note: {e}")

if __name__ == "__main__":
    generate_hardware_review()
