...
<body>
  ...
  <div class="main-container">
    ...
    <!-- Section 4.1.6: File Operations & System Calls (Interactive Kernel Debugger Simulator with Scenarios) -->
    <div class="card">
      ...
      <div style="background: #ffffff; border: 1px solid var(--border); border-radius: 6px; padding: 16px; display: flex; flex-direction: column; align-items: center; gap: 14px; margin-top: 10px;">
        <div style="display: flex; justify-content: space-between; width: 100%; align-items: center; flex-wrap: wrap; gap: 8px;">
          <span style="font-family: var(--font-mono); font-size: 0.78rem; font-weight: 700; color: var(--text-muted); text-transform: uppercase;">Interactive Kernel Debugger Simulator: Real-World Workflows</span>
          <div style="display: flex; gap: 6px; flex-wrap: wrap;">
            <button onclick="setScenario(0)" id="btn-s0" style="font-size: 0.73rem; padding: 4px 8px;">1. creat() Config</button>
            <button onclick="setScenario(1)" id="btn-s1" class="btn-secondary" style="font-size: 0.73rem; padding: 4px 8px;">2. open() Logs</button>
            <button onclick="setScenario(2)" id="btn-s2" class="btn-secondary" style="font-size: 0.73rem; padding: 4px 8px;">3. read() Stream</button>
            <button onclick="setScenario(3)" id="btn-s3" class="btn-secondary" style="font-size: 0.73rem; padding: 4px 8px;">4. write() State</button>
            <button onclick="setScenario(4)" id="btn-s4" class="btn-secondary" style="font-size: 0.73rem; padding: 4px 8px;">5. lseek() DB</button>
            <button onclick="setScenario(5)" id="btn-s5" class="btn-secondary" style="font-size: 0.73rem; padding: 4px 8px;">6. close() Flush</button>
          </div>
        </div>
        ...
        <div id="simDesc" style="font-size: 0.88rem; color: var(--text); text-align: center; background: #f8fafc; border: 1px solid var(--border); padding: 12px; border-radius: 6px; width: 100%; line-height: 1.5;">
          Select a real-world scenario above to simulate kernel subsystem interactions and data flow.
        </div>
      </div>
    </div>
  ...
  <script>
    ...
    function setScenario(index) {
      ...
      if (index === 0) {
        // creat() Config File
        pkt.style.opacity = '1';
        pkt.setAttribute('cx', simCoordinates[2]);
        ...
        document.getElementById('sim-node-disk').classList.add('active-sim-node');
        document.getElementById('lbl-proc').textContent = "creat() Daemon";
        document.getElementById('lbl-fd').textContent = "fd = 3";
        document.getElementById('lbl-offset').textContent = "Offset: 0B";
        document.getElementById('lbl-cache').textContent = "Cache Allocated";
        document.getElementById('lbl-disk').textContent = "i-node #512 Created";
        desc.innerHTML = "<strong>Scenario 1 (`creat()`):</strong> System daemon provisions <code>/etc/app.conf</code>. The kernel allocates a fresh i-node on disk and returns file descriptor 3 with initial permissions <code>0644</code>.";
      } else if (index === 1) {
        // open() Log Ingestion
        pkt.style.opacity = '1';
        pkt.setAttribute('cx', simCoordinates[1]);
        document.getElementById('sim-node-process').classList.add('active-sim-node');
        document.getElementById('sim-node-fd').classList.add('active-sim-node');
        document.getElementById('sim-node-inode').classList.add('active-sim-node');
        document.getElementById('lbl-proc').textContent = "Worker Process";
        document.getElementById('lbl-fd').textContent = "fd = 4 (Append)";
        document.getElementById('lbl-offset').textContent = "Offset: EOF";
        document.getElementById('lbl-cache').textContent = "Shared RAM Table";
        document.getElementById('lbl-disk').textContent = "Log Path Bound";
        desc.innerHTML = "<strong>Scenario 2 (`open()`):</strong> NGINX worker opens <code>access.log</code> with <code>O_APPEND</code>. The kernel establishes an open file table entry pointing write pointers directly to the end of the file.";
      } else if (index === 2) {
        // read() Streaming
        pkt.style.opacity = '1';
        pkt.setAttribute('cx', simCoordinates[3]);
        document.getElementById('sim-node-fd').classList.add('active-sim-node');
        document.getElementById('sim-node-inode').classList.add('active-sim-node');
        document.getElementById('sim-node-cache').classList.add('active-sim-node');
        document.getElementById('lbl-proc').textContent = "Compiler Loop";
        document.getElementById('lbl-fd').textContent = "fd = 3 active";
        document.getElementById('lbl-offset').textContent = "Offset: +4096B";
        document.getElementById('lbl-cache').textContent = "Cache Hit (RAM)";
        document.getElementById('lbl-disk').textContent = "Blocks Streamed";
        desc.innerHTML = "<strong>Scenario 3 (`read()`):</strong> Compiler streams source files in 4 KB chunks. Data is copied directly from the memory-resident <strong>Buffer Cache</strong>, automatically advancing the file offset.";
      } else if (index === 3) {
        ...
        document.getElementById('sim-node-inode').classList.add('active-sim-node');
        document.getElementById('sim-node-cache').classList.add('active-sim-node');
        document.getElementById('lbl-proc').textContent = "Editor Auto-Save";
        document.getElementById('lbl-fd').textContent = "fd = 3 active";
        document.getElementById('lbl-offset').textContent = "Offset Updated";
        document.getElementById('lbl-cache').textContent = "Dirty Cache Block";
        document.getElementById('lbl-disk').textContent = "Pending Flush";
        desc.innerHTML = "<strong>Scenario 4 (`write()`):</strong> Text editor saves user document changes. Data payloads are written into volatile buffer cache blocks, marking them dirty and updating i-node size metrics.";
      } else if (index === 4) {
        // lseek() Database Lookup
        pkt.style.opacity = '1';
        pkt.setAttribute('cx', simCoordinates[2]);
        document.getElementById('sim-node-process').classList.add('active-sim-node');
        document.getElementById('sim-node-inode').classList.add('active-sim-node');
        document.getElementById('lbl-proc').textContent = "DB Query Engine";
        document.getElementById('lbl-fd').textContent = "fd = 5 (Record #504)";
        document.getElementById('lbl-offset').textContent = "Offset: 245760B";
        document.getElementById('lbl-cache').textContent = "Cache Active";
        document.getElementById('lbl-disk').textContent = "Zero Disk I/O";
        desc.innerHTML = "<strong>Scenario 5 (`lseek()`):</strong> Database engine queries product record #504. <code>lseek()</code> instantly repositions the logical byte offset pointer in RAM without performing any physical disk head movement.";
      } else if (index === 5) {
        // close() Resource Cleanup
        pkt.style.opacity = '1';
        pkt.setAttribute('cx', simCoordinates[4]);
        document.querySelectorAll('.sim-node').forEach(n => n.classList.add('active-sim-node'));
        document.getElementById('lbl-proc').textContent = "Task Complete";
        document.getElementById('lbl-fd').textContent = "fd Released";
        document.getElementById('lbl-offset').textContent = "Offset Cleared";
        document.getElementById('lbl-cache').textContent = "Cache Flushed";
        document.getElementById('lbl-disk').textContent = "Persisted to SSD";
        desc.innerHTML = "<strong>Scenario 6 (`close()`):</strong> Utility finishes archiving bundle and calls <code>close()</code>. The kernel flushes all dirty cache blocks to persistent storage media and deallocates process-local descriptors.";
      }
    }
  </script>
</body>
...
COMMIT_MSG = """Expand kernel simulator with all 6 real-world file operation scenarios

Update week10-file-management/01-files-abstraction.html to include
interactive simulation steps for creat, open, read, write, lseek, and
close operations based on practical systems engineering workflows."""
...
def execute_pipeline():
    ...
    print(f"Wrote module file with all 6 scenarios to {target_file}")

    run_git_step(["git", "add", target_file], "Staging scenarios update 01-files-abstraction.html")
    run_git_step(["git", "commit", "-a", "-m", COMMIT_MSG], "Committing changes")
    run_git_step(["git", "push", "origin", "main"], "Pushing main to origin")
    print("--> All-Scenarios Simulator Module 01 created, committed, and pushed successfully!")
