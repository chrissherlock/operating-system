#!/usr/bin/env python3
# =====================================================================
# fix.py: Add control register and page table definitions to Module 2
# =====================================================================
import os
import subprocess
...
GLOSSARY_DEFINITIONS_HTML = """        <!-- Glossary Definitions: CR Registers & Table Structures -->
...
def add_glossary_definitions():
    ...
    if "Key Definitions &amp; Hardware Terminology" in content:
        print("--> Glossary definitions already present.")
        return

    # Target the end of the instructions & scenario guide card inside the translation simulator
    target_marker = '<li><strong>Granularity Toggles:</strong> Switch between modes to compare standard pages, superpages, and page faults.</li>\n          </ol>\n        </div>'

    if target_marker in content:
        replacement = target_marker.replace('</ol>\n        </div>', '</ol>\n' + GLOSSARY_DEFINITIONS_HTML + '\n        </div>')
        content = content.replace(target_marker, replacement)
        print("--> Successfully added glossary definitions inside the briefing card.")
    else:
        print("--> Error: Could not locate target insertion point for glossary definitions.")
        return
    ...
    try:
        ...
        commit_msg = (
            "Add control register and page table definitions to Module 2 walkthrough\n\n"
            "Embed a structured glossary definition table covering CR3, CR4, and PML4\n"
            "within the address translation simulator briefing card in 02-hardware-review.html."
        )
        subprocess.run(["git", "commit", "-m", commit_msg], check=True)
        subprocess.run(["git", "push", "origin", "main"], check=True)
        print("--> Git sync completed successfully for glossary definitions!")
    except Exception as e:
        print(f"Git execution note: {e}")

if __name__ == "__main__":
    add_glossary_definitions()
