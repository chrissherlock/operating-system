#!/usr/bin/env python3
# =====================================================================
# restore_full_week1_intro.py: Restore complete Week 1 intro portal
# =====================================================================
import os
import subprocess
import sys

FULL_INTRO_HTML = r"""<!DOCTYPE html>
<html lang="en">
<head>
  <meta charset="UTF-8">
...
<body>
  ...
  <main>
    ...
    <section class="content-section">
      ...
      <p>
        The history of operating systems spans five distinct technological generations, closely tracking the evolution of underlying electronic components.
      </p>
      ...
      <p>
        The invention of the transistor introduced solid-state electronics, making computers reliable enough to manufacture and sell commercially. This era birthed <strong>batch systems</strong>, where jobs were written to punched cards, collected into batches by an operator, loaded into the computer via card readers, and executed sequentially using early monitor programs (the precursors to modern operating systems).
      </p>
      <div class="image-grid">
        <div class="image-card">
      ...
      <p>
        Integrated circuits (ICs) revolutionized computer architecture. The IBM System/360 introduced hardware architecture families capable of running both commercial and scientific workloads. To eliminate CPU idle time during slow I/O operations, <strong>multiprogramming</strong> was developed: memory was partitioned to hold multiple jobs concurrently. When one job awaited disk I/O, the CPU switched to another. Concurrently, <strong>time-sharing</strong> systems emerged, allowing multiple interactive users to access a central mainframe simultaneously.
      </p>

      <h3>Generation 4: Personal Computers &amp; Networks (1980–Present)</h3>
      <p>
        Large-scale integration (LSI) chips made personal computers economically viable. Operating systems shifted toward graphical user interfaces (GUIs), rich file abstractions, and local area network (LAN) integration, establishing modern desktop and server paradigms.
      </p>

      <h3>Generation 5: Mobile, Cloud, &amp; Ubiquitous Computing (Present)</h3>
      <p>
        Modern operating systems power smartphones, tablets, edge sensors, and massive hyperscale cloud datacenters, emphasizing power management, sensor integration, containerization, and distributed virtualization.
      </p>

      <div class="aside-box">
        <strong>Research Aside: Andrew S. Tanenbaum &amp; MINIX</strong>
        <p style="margin-top: 6px; font-size: 0.88rem;">
          Andrew S. Tanenbaum created MINIX in 1987 as an educational operating system to illustrate the principles of microkernel design. MINIX served as the primary inspiration and initial development environment for Linus Torvalds when he began writing the Linux kernel in 1991.
        </p>
      </div>
    </section>
...
def restore_full_content():
    ...
    try:
        ...
        commit_msg = (
            "Restore complete introductory text, generations, research aside, and all 15 logos\n\n"
            "Update week01-operating-system-concepts/intro/index.html to restore full Chapter 1.1 and 1.2\n"
            "pedagogical content, generation image cards, and the 15-logo architectural family grid."
        )
        subprocess.run(["git", "commit", "-m", commit_msg], check=True)
        subprocess.run(["git", "push", "origin", "main"], check=True)
    except Exception:
        pass
    print("--> Complete introductory content successfully restored and deployed!")
