      ...
      <div style="background: #ffffff; border: 1px solid var(--border); border-radius: 6px; padding: 16px; display: flex; flex-direction: column; align-items: center; gap: 14px; margin-top: 10px;">
        <div style="display: flex; justify-content: space-between; width: 100%; align-items: center; flex-wrap: wrap; gap: 8px;">
          <div style="display: flex; gap: 4px; flex-wrap: wrap;">
            <button onclick="loadScenario(0)" id="sc-btn-0" style="font-size: 0.72rem; padding: 4px 8px;">1. creat() Config</button>
            <button onclick="loadScenario(1)" id="sc-btn-1" class="btn-secondary" style="font-size: 0.72rem; padding: 4px 8px;">2. open() Logs</button>
            <button onclick="loadScenario(2)" id="sc-btn-2" class="btn-secondary" style="font-size: 0.72rem; padding: 4px 8px;">3. read() Stream</button>
            <button onclick="loadScenario(3)" id="sc-btn-3" class="btn-secondary" style="font-size: 0.72rem; padding: 4px 8px;">4. write() State</button>
            <button onclick="loadScenario(4)" id="sc-btn-4" class="btn-secondary" style="font-size: 0.72rem; padding: 4px 8px;">5. lseek() DB</button>
            <button onclick="loadScenario(5)" id="sc-btn-5" class="btn-secondary" style="font-size: 0.72rem; padding: 4px 8px;">6. close() Flush</button>
          </div>
          <div style="display: flex; gap: 6px;">
            <button onclick="microStep(-1)" class="btn-secondary" style="font-size: 0.75rem; padding: 4px 10px;">&larr; Prev Microstep</button>
            <button onclick="microStep(1)" style="font-size: 0.75rem; padding: 4px 10px;">Next Microstep &rarr;</button>
          </div>
        </div>
        ...
        <div id="microDesc" style="font-size: 0.88rem; color: var(--text); text-align: center; background: #f8fafc; border: 1px solid var(--border); padding: 12px; border-radius: 6px; width: 100%; line-height: 1.5;">
          Select a scenario above and click <strong>"Next Microstep"</strong> to trace kernel execution.
        </div>
      </div>
    </div>

    <!-- Section Interactive Tour: The Anatomy of a File Descriptor -->
    <div class="card tour-panel" id="tourCard">
      <div class="tour-header">
        <span id="tourStepCounter">Stage 1 of 4: The Handshake</span>
        <span>Interactive Kernel Telemetry Tour</span>
      </div>
      <div id="tourTitle" class="tour-title">1. Opening the Gateway: The File Descriptor Allocation</div>
      <div id="tourText" class="tour-body">
        When a user-space process calls <code>open("data.txt", O_RDONLY)</code>, the kernel performs a security handshake. It traverses the directory tree, inspects i-node permission bits, and allocates a small integer entry in the process-local <strong>File Descriptor Table</strong>.
      </div>

      <!-- Live SVG Kernel Telemetry Table -->
      <div style="background: #ffffff; border: 1px solid #7dd3fc; border-radius: 6px; padding: 16px; display: flex; flex-direction: column; align-items: center; gap: 10px; margin-top: 10px;">
        <span style="font-family: var(--font-mono); font-size: 0.78rem; font-weight: 700; color: #0284c7; text-transform: uppercase;" id="tourDiagramCaption">Figure: Live Kernel Telemetry Table State (FD → OFT → Cache)</span>
        <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 680 150" width="100%" height="100%" style="font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif;">
          <!-- Table Header Background -->
          <rect x="20" y="15" width="640" height="26" fill="#0284c7" rx="4"/>
          <text x="100" y="32" font-size="10" font-weight="700" fill="#ffffff" text-anchor="middle">SUB-SYSTEM</text>
          <text x="260" y="32" font-size="10" font-weight="700" fill="#ffffff" text-anchor="middle">ACTIVE TELEMETRY OBJECT</text>
          <text x="520" y="32" font-size="10" font-weight="700" fill="#ffffff" text-anchor="middle">LIVE KERNEL VALUE / STATUS</text>

          <!-- Row 1: FD Table -->
          <rect x="20" y="45" width="640" height="30" fill="#f0f9ff" stroke="#bae6fd" stroke-width="1"/>
          <text x="100" y="64" font-size="10" font-weight="600" fill="#0369a1" text-anchor="middle">Process FD Table</text>
          <text x="260" y="64" font-size="10" fill="#334155" text-anchor="middle">Descriptor Slot Index</text>
          <text x="520" y="64" font-size="10" font-weight="700" fill="#0284c7" text-anchor="middle" id="svg-tbl-fd">fd [3] → Bound</text>

          <!-- Row 2: Open File Table -->
          <rect x="20" y="77" width="640" height="30" fill="#ffffff" stroke="#bae6fd" stroke-width="1"/>
          <text x="100" y="96" font-size="10" font-weight="600" fill="#0369a1" text-anchor="middle">Open File Table</text>
          <text x="260" y="96" font-size="10" fill="#334155" text-anchor="middle">Logical Byte Offset Pointer</text>
          <text x="520" y="96" font-size="10" font-weight="700" fill="#d97706" text-anchor="middle" id="svg-tbl-offset">Offset: 0B</text>

          <!-- Row 3: Buffer Cache / I-Node -->
          <rect x="20" y="109" width="640" height="30" fill="#f0f9ff" stroke="#bae6fd" stroke-width="1"/>
          <text x="100" y="128" font-size="10" font-weight="600" fill="#0369a1" text-anchor="middle">Buffer Cache / i-Node</text>
          <text x="260" y="128" font-size="10" fill="#334155" text-anchor="middle">RAM Block &amp; Metadata Status</text>
          <text x="520" y="128" font-size="10" font-weight="700" fill="#059669" text-anchor="middle" id="svg-tbl-cache">i-node #512 (RAM Resident)</text>
        </svg>
      </div>
      ...
      <div class="telemetry-grid">
        <div class="telemetry-box">
          ...
          <span class="telemetry-value" id="tel-fd">fd [3] → Bound</span>
        </div>
        <div class="telemetry-box">
          <span class="telemetry-label">Open File Table Entry</span>
          <span class="telemetry-value" id="tel-oft">Ref Count: 1 | Offset: 0B</span>
        </div>
        <div class="telemetry-box">
          <span class="telemetry-label">In-Memory I-Node</span>
          <span class="telemetry-value" id="tel-inode">i-node #512 (Cached in RAM)</span>
        </div>
      </div>

      <div class="tour-nav">
        <button id="tourPrevBtn" class="btn-secondary" disabled>&larr; Previous Stage</button>
        <button id="tourNextBtn" onclick="nextTourStage()">Next Stage &rarr;</button>
      </div>
    </div>
...
<span class="c-type">int</span> <span class="c-fn">main</span>(<span class="c-type">int</span> argc, <span class="c-type">char</span> *argv[]) {
    ...
    <div class="card sandbox-panel">
      ...
      <div class="sandbox-controls">
        ...
        <button class="btn-secondary" onclick="sandboxExec('read')">read()</button>
  ...
  <script>
    const tourStages = [
      {
        ...
        svgFd: "fd [3] → Bound",
        svgOffset: "Offset: 0B",
        svgCache: "i-node #512 (RAM Resident)",
        caption: "Figure: Live Kernel Telemetry Table State (FD → OFT → Cache)"
      },
      {
        ...
        svgFd: "fd [3] → Active",
        svgOffset: "Offset: 4096B (Advanced)",
        svgCache: "Block #104 (Cache Hit)",
        caption: "Figure: Live Telemetry Table State during read() Stream"
      },
      {
        ...
        svgFd: "fd [3] → Active",
        svgOffset: "Offset: 1024B (Seeked)",
        svgCache: "Zero Disk I/O (RAM Only)",
        caption: "Figure: Live Telemetry Table State during lseek() Repositioning"
      },
      {
        ...
        svgFd: "fd [3] → Released",
        svgOffset: "Offset: Cleared",
        svgCache: "Flushed & Synced to Disk",
        caption: "Figure: Live Telemetry Table State after close() and Sync"
      }
    ];

    let currentTourStage = 0;

    function nextTourStage() {
      if (currentTourStage < tourStages.length - 1) {
        currentTourStage++;
        updateTourDisplay();
      } else {
        currentTourStage = 0;
    ...
    function updateTourDisplay() {
      const stage = tourStages[currentTourStage];
      document.getElementById("tourStepCounter").textContent = stage.counter;
      document.getElementById("tourTitle").textContent = stage.title;
      document.getElementById("tourText").innerHTML = stage.text;
      document.getElementById("tel-fd").textContent = stage.fd;
      document.getElementById("tel-oft").textContent = stage.oft;
      document.getElementById("tel-inode").textContent = stage.inode;

      document.getElementById("svg-tbl-fd").textContent = stage.svgFd;
      document.getElementById("svg-tbl-offset").textContent = stage.svgOffset;
      document.getElementById("svg-tbl-cache").textContent = stage.svgCache;
      document.getElementById("tourDiagramCaption").textContent = stage.caption;
    ...
    function sandboxExec(cmd) {
      ...
      if (cmd === 'creat') {
        ...
        term.textContent = "$ read(3, buffer, 4096);\n[Kernel] Copied 256 bytes from buffer cache block #104 into user buffer. Offset advanced to 256.\nStatus: Success (Bytes read: 256)";
      } else if (cmd === 'write') {
        term.textContent = "$ write(3, \"TIMEOUT=30\\n\", 11);\n[Kernel] Copied 11 bytes into buffer cache block #104 at offset 256. File size updated to 11 bytes.\nStatus: Success (Bytes written: 11)";
      } else if (cmd === 'seek') {
    ...
    const scenarios = [
      {
        name: "1. creat() Config",
        steps: [
          { node: 0, text: "Microstep 1/5: Daemon process invokes <code>creat(\"/etc/app.conf\", 0644)</code>.", proc: "Calling creat()", fd: "fd = --", offset: "Offset: --", cache: "Clean", disk: "Ready" },
          { node: 0, text: "Microstep 2/5: Kernel traps system call and validates parent directory write permissions.", proc: "Validating Perms", fd: "fd = --", offset: "Offset: --", cache: "Clean", disk: "Ready" },
          { node: 4, text: "Microstep 3/5: File system allocates new i-node on disk and adds directory entry mapping.", proc: "Allocating i-node", fd: "fd = --", offset: "Offset: --", cache: "Allocated", disk: "i-node #512 Created" },
          { node: 2, text: "Microstep 4/5: Kernel establishes initial metadata (owner UID, permissions 0644, timestamps).", proc: "Writing Metadata", fd: "fd = --", offset: "Offset: 0B", cache: "Allocated", disk: "Attributes Set" },
          { node: 1, text: "Microstep 5/5: Kernel assigns file descriptor <code>fd = 3</code> to process and returns success.", proc: "Complete", fd: "fd = 3 Active", offset: "Offset: 0B", cache: "Ready", disk: "Persisted" }
        ]
      },
      {
        name: "2. open() Logs",
        steps: [
          { node: 0, text: "Microstep 1/6: Worker process invokes <code>open(\"access.log\", O_APPEND)</code>.", proc: "Calling open()", fd: "fd = --", offset: "Offset: --", cache: "Clean", disk: "Ready" },
          { node: 4, text: "Microstep 2/6: Kernel resolves directory path and locates existing i-node for log.", proc: "Path Lookup", fd: "fd = --", offset: "Offset: --", cache: "Checking", disk: "i-node Located" },
          { node: 2, text: "Microstep 3/6: Kernel validates write permissions against process credentials.", proc: "Checking Perms", fd: "fd = --", offset: "Offset: --", cache: "RAM Cached", disk: "Verified" },
          { node: 2, text: "Microstep 4/6: Open file table entry is created in kernel memory, loading i-node.", proc: "Table Entry", fd: "fd = --", offset: "Offset: EOF", cache: "Active", disk: "Bound" },
          { node: 2, text: "Microstep 5/6: Kernel evaluates <code>O_APPEND</code> flag, setting offset pointer to absolute end.", proc: "Appending", fd: "fd = --", offset: "Offset: EOF", cache: "Active", disk: "Bound" },
          { node: 1, text: "Microstep 6/6: Process-local file descriptor <code>fd = 4</code> allocated pointing to table.", proc: "Complete", fd: "fd = 4 Active", offset: "Offset: EOF", cache: "Ready", disk: "Ready" }
        ]
      },
      {
        name: "3. read() Stream",
        steps: [
          { node: 0, text: "Microstep 1/6: Compiler process invokes <code>read(fd, buffer, 4096)</code>.", proc: "Calling read()", fd: "fd = 3", offset: "Offset: Start", cache: "Clean", disk: "Ready" },
          { node: 1, text: "Microstep 2/6: Kernel verifies that file descriptor <code>fd = 3</code> is open for reading.", proc: "Validating FD", fd: "fd = 3 Valid", offset: "Offset: Start", cache: "Clean", disk: "Ready" },
          { node: 2, text: "Microstep 3/6: Kernel checks current offset pointer and resolves i-node block mapping.", proc: "Translating", fd: "fd = 3 Active", offset: "Offset: Active", cache: "Checking", disk: "Mapping" },
          { node: 3, text: "Microstep 4/6: Kernel checks Buffer Cache; cache hit bypasses physical disk access.", proc: "Cache Lookup", fd: "fd = 3 Active", offset: "Offset: Active", cache: "Cache Hit!", disk: "Bypassed" },
          { node: 0, text: "Microstep 5/6: Kernel copies 4096 bytes directly from kernel space into user buffer.", proc: "Copying Data", fd: "fd = 3 Active", offset: "Offset: Active", cache: "Copied", disk: "Ready" },
          { node: 2, text: "Microstep 6/6: Kernel automatically advances logical file offset pointer by bytes read.", proc: "Complete", fd: "fd = 3 Active", offset: "Offset: +4096B", cache: "Ready", disk: "Ready" }
        ]
      },
      {
        name: "4. write() State",
        steps: [
          { node: 0, text: "Microstep 1/6: Text editor process invokes <code>write(fd, buffer, 26)</code>.", proc: "Calling write()", fd: "fd = 3", offset: "Offset: Current", cache: "Clean", disk: "Ready" },
          { node: 2, text: "Microstep 2/6: Kernel verifies write permissions and checks current file offset pointer.", proc: "Validating", fd: "fd = 3 Active", offset: "Offset: Current", cache: "Clean", disk: "Ready" },
          { node: 3, text: "Microstep 3/6: Kernel locates target buffer cache block corresponding to offset.", proc: "Locating Block", fd: "fd = 3 Active", offset: "Offset: Current", cache: "Block Ready", disk: "Ready" },
          { node: 3, text: "Microstep 4/6: Data payload bytes are copied from user space into buffer cache blocks.", proc: "Writing Cache", fd: "fd = 3 Active", offset: "Offset: Current", cache: "Cache Updated", disk: "Ready" },
          { node: 2, text: "Microstep 5/6: Buffer marked as <strong>dirty</strong>; in-memory i-node updates size and timestamp.", proc: "Marking Dirty", fd: "fd = 3 Active", offset: "Offset: Current", cache: "Dirty Buffer", disk: "Pending" },
          { node: 2, text: "Microstep 6/6: Logical offset pointer advances by number of bytes written.", proc: "Complete", fd: "fd = 3 Active", offset: "Offset: Advanced", cache: "Dirty Buffer", disk: "Pending" }
        ]
      },
      {
        name: "5. lseek() DB",
        steps: [
          { node: 0, text: "Microstep 1/5: DB engine needs record #504 at byte <code>245760</code>, calls <code>lseek()</code>.", proc: "Calling lseek()", fd: "fd = 5", offset: "Offset: 0B", cache: "Clean", disk: "Ready" },
          { node: 2, text: "Microstep 2/5: Kernel validates target offset against maximum permissible file size limit.", proc: "Validating Limit", fd: "fd = 5 Active", offset: "Offset: Validated", cache: "Clean", disk: "Ready" },
          { node: 2, text: "Microstep 3/5: Kernel updates logical byte offset pointer inside Open File Table to <code>245760</code>.", proc: "Updating Offset", fd: "fd = 5 Active", offset: "Offset: 245760B", cache: "Clean", disk: "Ready" },
          { node: 4, text: "Microstep 4/5: <strong>Zero disk I/O occurs</strong>; no physical storage head movement triggered.", proc: "Zero Disk I/O", fd: "fd = 5 Active", offset: "Offset: 245760B", cache: "Bypassed", disk: "No Movement" },
          { node: 0, text: "Microstep 5/5: Subsequent read or write operations execute immediately at new position.", proc: "Complete", fd: "fd = 5 Active", offset: "Offset: 245760B", cache: "Ready", disk: "Ready" }
        ]
      },
      {
        name: "6. close() Flush",
        steps: [
          { node: 0, text: "Microstep 1/5: Utility process invokes <code>close(fd)</code>.", proc: "Calling close()", fd: "fd = 3", offset: "Offset: Active", cache: "Dirty", disk: "Pending" },
          { node: 2, text: "Microstep 2/5: Kernel checks open file table reference count associated with descriptor.", proc: "Checking Refs", fd: "fd = 3 Active", offset: "Offset: Active", cache: "Dirty", disk: "Pending" },
          { node: 4, text: "Microstep 3/5: Kernel flushes dirty cache blocks out to physical storage media (disks/SSDs).", proc: "Flushing Cache", fd: "fd = 3 Active", offset: "Offset: Active", cache: "Flushing...", disk: "Writing..." },
          { node: 1, text: "Microstep 4/5: Kernel deallocates process-local file descriptor slot in per-process FD table.", proc: "Deallocating FD", fd: "fd Released", offset: "Offset: Cleared", cache: "Clean", disk: "Synced" },
          { node: 2, text: "Microstep 5/5: Open file table reference count decremented; i-node reference released.", proc: "Complete", fd: "Closed", offset: "Cleared", cache: "Clean", disk: "Persisted" }
        ]
      }
    ];
    ...
    function loadScenario(index) {
      ...
      renderMicrostepState();
    }

    function microStep(dir) {
      const maxSteps = scenarios[currentScenario].steps.length;
      currentMicrostep += dir;
      if (currentMicrostep < 0) currentMicrostep = 0;
      if (currentMicrostep >= maxSteps) currentMicrostep = maxSteps - 1;
      renderMicrostepState();
    }

    const microCoords = [75, 225, 385, 545, 690];

    function renderMicrostepState() {
      ...
      for (let i = 0; i < 5; i++) {
        ...
        if (i === step.node) {
          el.classList.add('active-sim-node');
        } else {
          el.classList.remove('active-sim-node');
        }
      }
...
COMMIT_MSG = """Add live SVG telemetry table to file descriptor tour in module 01

Update week10-file-management/01-files-abstraction.html to replace static
diagrams with a live, vector SVG telemetry table that dynamically updates
kernel table values during guided tour steps."""
...
def execute_pipeline():
    ...
    print(f"Wrote module file with live SVG telemetry table to {target_file}")

    run_git_step(["git", "add", target_file], "Staging SVG telemetry table update 01-files-abstraction.html")
    run_git_step(["git", "commit", "-a", "-m", COMMIT_MSG], "Committing changes")
    run_git_step(["git", "push", "origin", "main"], "Pushing main to origin")
    print("--> SVG Telemetry Table Module 01 created, committed, and pushed successfully!")
