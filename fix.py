...
<head>
  ...
  <style>
    ...
    * { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    /* =========================================================
       THEME 4: MS-DOS 6.22 DEFRAG (COMPACT 800x600 FIT)
       ========================================================= */
    .theme-olddos {
      background-color: #0000aa;
      color: #ffffff;
    ...
    .theme-olddos .ui-title { color: #0000aa; font-size: 10px; font-weight: bold; }
    ...
    .theme-olddos .screen-grid {
      ...
      width: 100%;
      max-width: 950px;
      height: 165px; /* Compact height ensures zero vertical scroll on 800x600 */
    }
    .theme-olddos .c-cell {
      aspect-ratio: 1 / 1.5;
      border-radius: 0;
...
<body>
  ...
  <div class="main-container">
    ...
    <!-- QUAD-THEME MULTI-CAPACITY FAT DEFRAGMENTER SIMULATOR -->
      ...
      <div class="ui-window-box">
        ...
        <div class="dos-legend-box" id="dosLegendBox">
          <div class="dos-status-col">
            ...
            <div style="text-align:center; font-size:8.5px;" id="dosElapsedText">Elapsed Time: 00:00:00</div>
            <div style="text-align:center; font-size:8.5px; font-weight:bold; color:#ffff55;" id="dosOptModeText">Full Optimization</div>
          </div>
          <div class="dos-legend-col">
            ...
            <div style="display:grid; grid-template-columns: 1fr 1fr; gap:1px; font-size:8px;">
              <div>■ - Used</div>
              <div>▒ - Unused</div>
              <div>r - Reading</div>
              <div>W - Writing</div>
            ...
            <div style="margin-top:auto; font-size:8px; color:#55ffff;" id="dosDriveBlockText">Drive C:  1 block = 54 clusters</div>
          </div>
        </div>
  ...
  <script>
    ...
    function defragInitVolume() {
      ...
      const totalClusters = selectedCapacityMB * 256;
      const ratio = Math.round(totalClusters / TOTAL_CELLS);
    ...
    function defragHeavyChurn() {
      ...
      const capLabel = selectedCapacityMB >= 1000 ? "1 GB" : `${selectedCapacityMB} MB`;
      document.getElementById("txtStatusMsg").textContent =
        `Drive C: (${capLabel}) Heavily Fragmented (64% Fragmentation). High seek latency detected!`;
    }
    ...
    function defragToggleRun() {
      if (isRunning) {
        ...
        const capLabel = selectedCapacityMB >= 1000 ? "1 GB" : `${selectedCapacityMB} MB`;
        document.getElementById("txtStatusMsg").textContent = `Defragmenting ${capLabel} Volume... Gathering scattered cluster runs.`;
        runDefragCycle();
      }
    }
      ...
      stepTimer = setTimeout(() => {
        ...
        stepTimer = setTimeout(() => {
          ...
          targetSlots.forEach(idx => {
            cells[idx] = { state: "optimized", isSystem: false, fileId: "opt" };
            renderCell(idx);
          });
...
COMMIT_MSG = """Fix 800x600 vertical overflow for MS-DOS 6.22 theme in defrag simulator

Update week10-file-management/03-filesystem-implementation.html to tighten
grid height and padding for the MS-DOS 6.22 theme so the entire interface
fits comfortably without vertical scrolling on 800x600 displays."""
...
def deploy_module():
    ...
    run_git_step(["git", "add", target_file], "Staging MS-DOS 6.22 overflow fix")
    run_git_step(["git", "commit", "-a", "-m", COMMIT_MSG], "Committing changes")
    run_git_step(["git", "push", "origin", "main"], "Pushing main to origin")
    print("--> MS-DOS 6.22 overflow fix successfully deployed!")
