...
<head>
  ...
  <style>
    ...
    * { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    .sm-subtext { font-size: 8px; fill: #64748b; text-anchor: middle; }
    .active-state .sm-box { fill: #0284c7; stroke: #0369a1; stroke-width: 3px; filter: drop-shadow(0 4px 8px rgba(2,132,199,0.3)); }
  ...
  <div class="main-container">
    ...
    <div class="card tour-panel" id="tourCard">
      <div class="tour-header">
        ...
        <span>Interactive Kernel State Machine Tour</span>
      </div>
      ...
      <div style="background: #ffffff; border: 1px solid #7dd3fc; border-radius: 6px; padding: 16px; display: flex; flex-direction: column; align-items: center; gap: 10px; margin-top: 10px;">
        <span style="font-family: var(--font-mono); font-size: 0.78rem; font-weight: 700; color: #0284c7; text-transform: uppercase;" id="tourDiagramCaption">Figure: File Descriptor Lifecycle State Machine</span>
        <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 720 140" width="100%" height="100%" style="font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif;">
          <!-- State 0: UNALLOCATED -->
          <g id="sm-state-0" class="sm-state active-state" onclick="jumpTourStage(0)">
            <rect x="20" y="35" width="110" height="60" class="sm-box"/>
            <text x="75" y="60" class="sm-text">UNALLOCATED</text>
            <text x="75" y="76" class="sm-subtext">Process Closed</text>
          </g>

          <path d="M 130 65 L 180 65" stroke="#0284c7" stroke-width="2" marker-end="url(#smarrow)"/>
          <text x="155" y="55" font-size="8" fill="#0369a1" font-weight="600" text-anchor="middle">open()</text>

          <!-- State 1: FD_ALLOCATED -->
          <g id="sm-state-1" class="sm-state" onclick="jumpTourStage(1)">
            <rect x="180" y="35" width="120" height="60" class="sm-box"/>
            <text x="240" y="60" class="sm-text">FD_ALLOCATED</text>
            <text x="240" y="76" class="sm-subtext">fd slot bound</text>
          </g>

          <path d="M 300 65 L 350 65" stroke="#0284c7" stroke-width="2" marker-end="url(#smarrow)"/>
          <text x="325" y="55" font-size="8" fill="#0369a1" font-weight="600" text-anchor="middle">read()</text>

          <!-- State 2: OFT_BOUND -->
          <g id="sm-state-2" class="sm-state" onclick="jumpTourStage(2)">
            <rect x="350" y="35" width="110" height="60" class="sm-box"/>
            <text x="405" y="60" class="sm-text">OFT_BOUND</text>
            <text x="405" y="76" class="sm-subtext">Offset advancing</text>
          </g>

          <path d="M 460 65 L 510 65" stroke="#0284c7" stroke-width="2" marker-end="url(#smarrow)"/>
          <text x="485" y="55" font-size="8" fill="#0369a1" font-weight="600" text-anchor="middle">lseek()</text>

          <!-- State 3: RAM_CACHED -->
          <g id="sm-state-3" class="sm-state" onclick="jumpTourStage(3)">
            <rect x="510" y="35" width="110" height="60" class="sm-box"/>
            <text x="565" y="60" class="sm-text">RAM_CACHED</text>
            <text x="565" y="76" class="sm-subtext">Random Access</text>
          </g>

          <!-- Return arrow down and back for close() -->
          <path d="M 565 95 L 565 115 L 75 115 L 75 95" fill="none" stroke="#d97706" stroke-width="2" stroke-dasharray="4,4" marker-end="url(#smarrow-close)"/>
          <text x="320" y="128" font-size="8" fill="#d97706" font-weight="700" text-anchor="middle">close() [Flushes Cache &amp; Deallocates FD]</text>
...
<span class="c-type">int</span> <span class="c-fn">main</span>(<span class="c-type">int</span> argc, <span class="c-type">char</span> *argv[]) {
  ...
  <script>
    const tourStages = [
      {
        ...
        stateIndex: 0,
        caption: "Figure: State Machine in [UNALLOCATED] → Transitioning via open()"
      },
      {
        ...
        stateIndex: 1,
        caption: "Figure: State Machine in [FD_ALLOCATED] → Transitioning via read()"
      },
      {
        ...
        stateIndex: 2,
        caption: "Figure: State Machine in [OFT_BOUND] → Transitioning via lseek()"
      },
      {
        ...
        stateIndex: 3,
        caption: "Figure: State Machine returning to [UNALLOCATED] via close()"
      }
    ];
    ...
    function jumpTourStage(index) {
      currentTourStage = index;
      updateTourDisplay();
    }
...
COMMIT_MSG = """Replace telemetry table with interactive kernel state machine diagram

Update week10-file-management/01-files-abstraction.html to feature an
interactive SVG state machine diagram illustrating file descriptor lifecycle
states, kernel handshakes, and transition triggers for technical learners."""
...
def execute_pipeline():
    ...
    print(f"Wrote module file with state machine diagram to {target_file}")

    run_git_step(["git", "add", target_file], "Staging state machine update 01-files-abstraction.html")
    run_git_step(["git", "commit", "-a", "-m", COMMIT_MSG], "Committing changes")
    run_git_step(["git", "push", "origin", "main"], "Pushing main to origin")
    print("--> State Machine Diagram Module 01 created, committed, and pushed successfully!")
